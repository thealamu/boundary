...
func TestRepository_CreateSet(t *testing.T) {
	...
	var tests = []struct {
		...
		},
		{
			...
			wantIsErr: errors.MissingCatalogId,
		},
		{
	...
	t.Run("invalid-duplicate-names", func(t *testing.T) {
		...
		assert.Truef(errors.Match(errors.T(errors.NotUnique), err), "want err: %v got: %v", errors.NotUnique, err)
		assert.Nil(got2)
	})
...
func TestRepository_UpdateSet(t *testing.T) {
	...
	var tests = []struct {
		...
		},
		{
			...
			masks:     []string{"Name", "Description"},
			wantIsErr: errors.MissingPublicId,
		},
		{
